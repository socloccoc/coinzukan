...
class browser extends uploader {
    ...
    protected function act_newDir() {
		...
		// Hải -> chuyển sang không dấu
		 $newDir = $this->aliasDir($newDir);
        if (!strlen($newDir))
	...
	public function removeUtf8Dir($value = NULL){
		$chars = array(
	...
	public function aliasDir($value = NULL){
		$value = $this->removeUtf8Dir($value);
