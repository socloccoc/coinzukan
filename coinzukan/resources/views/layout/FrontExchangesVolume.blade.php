...
@section('content')
    <div class="row subheader">
        <div class="col-xs-12 text-center">
            <h1>{{ trans('content.HOME_PAGE.24_hour_volume_rankings_exchange') }}</h1>
        </div>
    </div>
    <?php
    $i = 0;
            ...
            <div class="table-responsive">
                <table class="table no-border table-condensed">
                    ...
                    @foreach($listMarkets as $listMarket)
                        <tr id="bitcoin">
                            <td colspan="6" class="volume-header-container">
                                <h3 class="volume-header">1.
                                    <a href="">
                                        {{ isset($listMarket->market_name) && !empty($listMarket->market_name) ? ucwords($listMarket->market_name) : '' }}
                                    </a>
                                </h3>
                            </td>
                        </tr>
                        <tr>
                            <th>#</th>
                            <th>{{ trans('content.HOME_PAGE.currency') }}</th>
                        ...
                        {!! App::make('App\Http\Controllers\FrontEnd\HomeCustomController')->getListPairByMarketId($listMarket->id)!!}
                    @endforeach
                    </tbody>
                </table>
            </div>
