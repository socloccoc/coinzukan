...
@forelse($listCryptocurrencies as $row)
    ...
    <tr id="id-bitcoin" class="odd" role="row">
        ...
        <td class="no-wrap currency-name">
            ...
            <a href="{{ route('detail.coin', $row->coin_name) }}"
               title="{!! isset($row->coin_name) ? $row->coin_name : 'N/A' !!}">
        ...
        <td class="no-wrap text-right">
            <a href="currencies/{{ $row->coin_name }}/#markets_cap" class="volume">
                {!! isset($row->volume_24h) && !empty ($row->volume_24h) ? number_format($row->volume_24h, 0) . $unit : '0$'  !!}
            </a>
        </td>
