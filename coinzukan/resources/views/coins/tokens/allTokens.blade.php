...
@forelse($listTokens as $row)
    ...
    <tr id="id-bitcoin" class="odd" role="row">
        ...
        <td class="no-wrap percent-1h  {{isset($row->percent_1h) && $row->percent_1h > 0 ? 'positive_change' : 'negative_change' }} text-right"
    ...
    <div style="position: fixed; width: 2490px; height: 750px; left: 0px" hidden
         id="{{'container_'.preg_replace('/\s/u', '', $row->coin_name) }}">
    </div>
@empty
    <tr>
        <td colspan="8">
