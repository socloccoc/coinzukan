...
@forelse($listTokens as $row)
    ...
    <tr id="id-bitcoin" class="odd" role="row">
        ...
        <td class="no-wrap percent-24h  {{isset($row->percent_24h) && !empty($row->percent_24h) && $row->percent_24h > 0 ? 'positive_change' : 'negative_change' }} text-right"
            data-usd="-10.39" data-btc="0.00">
            {!! isset($row->percent_24h) && !empty($row->percent_24h) ? number_format($row->percent_24h, 2) . '%' : 0 !!}
        </td>

        <td>
            <a href="#">
                <img {{ isset($row->imageChart) && !empty($row->imageChart) ? 'src='.$row->imageChart : '' }}
